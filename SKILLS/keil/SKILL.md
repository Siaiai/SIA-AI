...
- **config.json 路径转义**：Windows 路径含反斜杠，JSON 字符串里必须写成 `\\`（如 `"E:\\software\\Keil\\UV4.exe"`）。写成单 `\` 会导致 `json.loads` 失败，脚本静默读不到配置，表现为"缺少必要参数: uv4"。`load_json_file` 已加解析失败 stderr 警告，看到即知 config 损坏。用 `python -c "import json;json.dump(...)"` 写 config 可自动转义。
- **config.json 已配置 uv4_exe 时无需传 `--uv4`**：脚本按优先级 CLI > config 自动解析。只在 config 未配或临时覆盖时才需 `--uv4`。
- **UV4 命令行无独立 clean**：`clean` 通过 `-c`（rebuild 的 clean-first）或删除输出目录实现，非 UV4 原生子命令。
